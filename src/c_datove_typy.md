# Datové typy
[Paměť](pamet.md) počítače umí pracovat pouze s jednotlivými *byty*, nicméně pro lidi
...
(1 - 8) bytů[^1]. Počet bytů udává, jak velký rozsah můžou hodnoty daného typu obsahovat. Například
...
znaménka, nezáporné). Tato vlastnost udává, jaký rozsah hodnot můžou hodnoty daného typu nabývat
(tj. jestli můžou být i záporné nebo ne). Například číslem o velikosti jednoho bytu můžeme
...
> v sekci o [řetězcích](c_retezce.md).
...
použít [odpovídající](#Desetinné-číselné-typy) datový typ.
- Jelikož mají čísla v počítači omezenou přesnost (několik jednotek bytů), tak může při matematických
...
které můžou změnit datový typ výrazů, které používáte, bez vašeho vědomí. Je tak (obzvláště ze začátku)
...
Například výsledek výrazu `1 + 2 * 3` je `7`, a ne `9`.
